...
import { IsNull, MoreThanOrEqual, Repository, UpdateResult } from "typeorm";
...
export class TicketRoleTechnicianStrategy implements TicketRoleStrategyInterface {
    ...
    async findAllFromDate(date: Date): Promise<Ticket[]> {
        return this.ticketRepository.find({ 
            where: [{technician: {id: this.user.id}, created_at: MoreThanOrEqual(date)}, {technician: IsNull(), created_at: MoreThanOrEqual(date)}], 
            relations: ["customer", "technician", "priority"],
            order: {created_at: "DESC"}  
        });
    }
